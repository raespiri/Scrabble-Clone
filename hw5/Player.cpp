...
void Player::movePlace(string dir, int row, int col, string tiles_to_place, Board &board, Dictionary dictionary, Bag &bag, int &count, int* error) {
	...
	for(unsigned int j = 0; j < tiles_to_place.length(); j++) {
...
string Player::getHand() {
	...
	string hand = ""; //empty hand
  	...
  	for(it = _hand.begin(); it != _hand.end(); it++) {
    	hand = hand + " " + (*it)->getLetter(); //grabbing letter from tile and adding it to hand string
	}
	return hand;
}
