...
int main (int nargs, char **args)
{
	...
	try {
		...
		readConfigFile (args[1],
						dictionaryFileName, boardFileName, bagFileName, initFileName,
						numTiles);

		Dictionary dict (dictionaryFileName);
		int turn_counter = 0; //counter for number of turns, used for start square check
		Board board (boardFileName, initFileName, turn_counter);
		...
		for(int i = 0; i < num_players; i++) {
			...
			if(name == "CPUS" || name == "cpus") {
				Player* new_player = new Player(name, &dict);
		...
		while(bag.tilesRemaining() > 0 && !emptyHand(playerList)) { //while there are still tiles in the bag and no one has an empty hand
			...
			for(unsigned int i = 0; i < playerList.size(); i++) { //loop through all the players
				cout << playerList[i]->getName() << "," << " it's your turn!" << endl;
				...
				for(unsigned int j = 0; j < playerList.size(); j++) {
					cout << playerList[j]->getName() << "'s score is: " << playerList[j]->getScore() << endl; //printing out the score for each player
				}
				int player_index = (int) i; //casting unsigned int to int
				playerList[player_index]->printHand(); //printing curr player's hand
				
				if(playerList[i]->getName() == "CPUS" || playerList[i]->getName() == "cpus") {
					playerList[i]->AIsolve(board, turn_counter);
					if(playerList[i]->getAImoves_size() == 0) {
						playerList[i]->movePass();
						all_pass_counter++;
					}
					else { 
						playerList[i]->movePlace(playerList[i]->AIgetMove()->dir, playerList[i]->AIgetMove()->row+1, 
											playerList[i]->AIgetMove()->col+1, playerList[i]->AIgetMove()->tiles,
											board, dict, bag, turn_counter);
					}
				}
				else if(playerList[i]->getName() == "CPUL" || playerList[i]->getName() == "cpul") {
					playerList[i]->AIsolve2(board, turn_counter);
					if(playerList[i]->getAImoves_size2() == 0) {
						playerList[i]->movePass();
						all_pass_counter++;
					}
					else {
						playerList[i]->movePlace(playerList[i]->AIgetMove2()->dir, playerList[i]->AIgetMove2()->row+1, 
											playerList[i]->AIgetMove2()->col+1, playerList[i]->AIgetMove2()->tiles,
											board, dict, bag, turn_counter);
					}
				}
				else {
			...
			if(all_pass_counter == num_players) { //if all the players passed
				int player_index = -1;
				int score = -1;
				for(int x = 0; x < (int) playerList.size(); ++x) {
					if(playerList[x]->getScore() > score) {
						score = playerList[x]->getScore();
						player_index = x;
					}
				}
				cout << playerList[player_index]->getName() << " won the game with a score of " << playerList[player_index]->getScore() << endl;
