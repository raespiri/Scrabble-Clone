...
class BinarySearchTree 
{
	...
	protected:
		...
		Node<Key, Value>* insertHelper(Node<Key, Value>* node, Node<Key, Value>* parent, const std::pair<Key, Value>& keyValuePair); 
...
{ 
	// TODO
	mCurrent = NULL; //iterator points to NULL if no parameter is passed
}
...
{ 
	// TODO
	mCurrent = ptr; //points to address of the  passed Node ptr
}
...
{
	// TODO
	return(mCurrent == rhs.mCurrent); //check if rhs and 'this' point to the same node
}
...
{
	// TODO
	if(mCurrent != rhs.mCurrent) { //returns true if 'this' and rhs point to different nodes
		return true;
	}
	return false; 
}
...
{
	// TODO
	mCurrent = rhs.mCurrent; //sets 'this' to point to same node as rhs
	return *this;
}
...
{
	// TODO
	if(mCurrent->getRight() == NULL && mCurrent->getParent() == NULL) { //last node in the BST
		mCurrent = NULL; //set iterator to NULL
		return *this;
	}
	else if(mCurrent->getRight() != NULL) { //if right child is not NULL
		mCurrent = mCurrent->getRight(); //point iterator to right child 
		if(mCurrent->getLeft() != NULL) { //if right child has a left child
			mCurrent = mCurrent->getLeft(); //point iterator to that child
			while(mCurrent->getLeft() != NULL) { //traverse as far down this new left subtree as possible
				mCurrent = mCurrent->getLeft(); //updating mCurrent as you go
			}
			return *this;
		}
		else { //else return
		...
		while(mCurrent->getParent() != NULL) { //traverse up parent nodes
			if(mCurrent->getParent()->getKey() > (*(*this)).first) { //if parent is a "left" node
				mCurrent = mCurrent->getParent(); //set iterator to this left parent
				return *this;
			}
			mCurrent = mCurrent->getParent();
		}
		//A left parent was unable to be found, so we must be at the last node in the BST
		mCurrent = NULL;  //set iterator to NULL
		return *this;
	}
}
...
typename BinarySearchTree<Key, Value>::iterator BinarySearchTree<Key, Value>::begin()
{
	...
	while(temp != NULL && temp->getLeft() != NULL) { //loop down left subtree
		temp = temp->getLeft(); //set it to point to left child
	}
	iterator it(temp);
	return it; //return updated iterator 
}
...
typename BinarySearchTree<Key, Value>::iterator BinarySearchTree<Key, Value>::end()
{
	// TODO
	iterator it;
	return it; //return NULL iterator 
}
...
{
	mRoot = insertHelper(mRoot, NULL, keyValuePair);
}

/**
* Insert helper function to recurse down BST comparing key of node to be inserted
* with current node.
*/
template<typename Key, typename Value>
Node<Key, Value>* BinarySearchTree<Key, Value>::insertHelper(Node<Key, Value>* node, Node<Key, Value>* parent, const std::pair<Key, Value>& keyValuePair)
{ 
	if(node == NULL) { //if node is NULL
		Node<Key, Value>* new_node = new Node<Key, Value>(keyValuePair.first, keyValuePair.second, parent); //create new node to be inserted
		return new_node;
	}

	if(keyValuePair.first == node->getKey()) { //if duplicate key found
		node->setValue(keyValuePair.second); //update value of key
		return node;
	}
	else if(keyValuePair.first > node->getKey()) { //if key is greater than current node
		node->setRight(insertHelper(node->getRight(), node, keyValuePair)); //move to right node
		return node;
	}
	else {
		node->setLeft(insertHelper(node->getLeft(), node, keyValuePair));
		return node;
	}
}
